...
// Browsers refuse to store a `Secure` cookie over plain http (which is how
// local dev normally runs), and `SameSite=None` additionally requires
// `Secure` to be set — so hardcoding both breaks session persistence on
// http://localhost. Base both on whether AUTH_URL is actually https.
